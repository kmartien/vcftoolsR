...
vcftools.idepth <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--depth"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.siteDepth <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--site-mean-depth"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.genoDepth <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--geno-depth"),
          stdout = FALSE)
}
...
vcftools.imiss <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--missing-indv"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.siteMiss <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--missing-site"),
          stdout = FALSE)
}
...
vcftools.freqBurden <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--indv-freq-burden"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.freq2 <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--freq2"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.singletons <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--singletons"),
          stdout = FALSE)
}

#' @rdname vcftools.idepth
vcftools.012 <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--012"),
          stdout = FALSE)
}
...
vcftools.het <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--het"),
          stdout = FALSE)
}
# SNP call quality
#' @rdname vcftools.idepth
vcftools.siteQual <- function(vcf.fn, res.fn){
  system2(command = "vcftools", args = c("--vcf ", paste0(vcf.fn,".vcf"), "--out ", res, "--site-quality"),
          stdout = FALSE)
}
